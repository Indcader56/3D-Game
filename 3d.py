...
while True:
    ...
    if click_w == True:
        new_x = player_x + math.cos(player_dir)*player_speed
        new_y = player_y + math.sin(player_dir)*player_speed
        if world_data[round(new_y)//tile_size_y][round(new_x)//tile_size_x] == 0:
            player_x = new_x
            player_y = new_y
    ...
    if mouse_down == True:
        mouse_block_x = round((mouse_x-window_size[0]/2)+player_x)//tile_size_x
        mouse_block_y = round((mouse_y-window_size[1]/2)+player_y)//tile_size_y

        if mouse_block_y < world_size_y and mouse_block_y >= 0:
            if mouse_block_x < world_size_x and mouse_block_x >= 0:
                world_data[round((mouse_y-window_size[1]/2)+player_y)//tile_size_y][round((mouse_x-window_size[0]/2)+player_x)//tile_size_x] = tile_select
    ...
    for x in range(240):
        ...
        while True:
            ...
            world_ray_x = round((ray_x+player_x)//tile_size_x)
            world_ray_y = round((ray_y+player_y)//tile_size_y)
    ...
    if hud == True:
        for y in range(world_size_y):
            for x in range(world_size_x):
                if world_data[y][x] == 1:
                    pygame.draw.rect(screen, map_color, (((tile_size_x*x)-player_x)+window_size[0]/2,((tile_size_y*y)-player_y)+window_size[1]/2,tile_size_x,tile_size_y))

        for ray in ray_lines:
            pygame.draw.line(screen, ray_color, (window_size[0]/2,window_size[1]/2), (ray[0]+window_size[0]/2, ray[1]+window_size[1]/2), 1)

        pygame.draw.circle(screen, player_color, (window_size[0]/2,window_size[1]/2), player_size)
        pygame.draw.line(screen, player_color, (window_size[0]/2,window_size[1]/2), ((math.cos(player_dir)*20)+window_size[0]/2,(math.sin(player_dir)*20)+window_size[1]/2))

        pygame.draw.rect(screen, map_color, ((-player_x)+window_size[0]/2,(-player_y)+window_size[1]/2,tile_size_x*world_size_x,tile_size_y*world_size_y), 4)
